      ...
      <div className="max-w-[1140px] mx-auto px-7 grid grid-cols-1 nav:grid-cols-[0.8fr_1.2fr] gap-14 items-start">
        <FadeUp className="relative">
          <div className="aspect-square rounded-[20px] relative overflow-hidden bg-linear-to-br from-panel to-panel-2 border border-border flex items-center justify-center">
            <span className="font-display text-[5rem] font-bold code-icon-gradient opacity-90">&lt;/&gt;</span>
          </div>
        ...
        <FadeUp>
          ...
          <p className="text-ink-muted mb-4 text-[1.02rem]">
            I&apos;m a <strong className="text-ink font-semibold">MERN stack developer</strong> who enjoys the
            full journey of building software — sketching a data model, wiring up a REST API, and polishing the
            last pixel of a React interface. I like projects that solve a real, everyday problem for real people.
          </p>
          <p className="text-ink-muted mb-4 text-[1.02rem]">
            My recent work includes a <strong className="text-ink font-semibold">Hospital Appointment System</strong>{' '}
            that cut manual booking errors for a small clinic, and a{' '}
            <strong className="text-ink font-semibold">Job Portal</strong> connecting applicants with recruiters
            through role-based dashboards. I care about code that&apos;s easy to read six months later, not just
            code that works today.
          </p>
          <p className="text-ink-muted mb-4 text-[1.02rem]">
            Outside of client work, I contribute to open-source utilities, write about JavaScript patterns, and
            I&apos;m always mid-way through breaking (and fixing) a side project.
          </p>
          <div className="grid grid-cols-2 nav:grid-cols-3 gap-4 mt-8">
            {FACTS.map((fact) => (
              <div key={fact.label} className="border border-border rounded-xl p-[18px] bg-panel">
                <div className={`font-display font-bold text-[1.5rem] ${fact.color}`}>{fact.num}</div>
                <div className="text-[0.8rem] text-ink-muted mt-1">{fact.label}</div>
              </div>
            ))}
          </div>
        </FadeUp>
